...
class AeroTwinRULModel(nn.Module):
    """
    Dual Encoder GRU for Remaining Useful Life (RUL) Estimation.
    Allows pretraining on a 14-sensor dataset (C-MAPSS) and fine-tuning
    on a 6-sensor dataset (Simulator Residuals) by sharing the core GRU physics engine.
    """
    def __init__(self, cmapss_dim=14, sim_dim=6, latent_dim=32, hidden_size=64):
        ...
        # The Shared Brain
        self.gru = nn.GRU(input_size=latent_dim, hidden_size=hidden_size, batch_first=True)
    ...
    def freeze_shared_core(self):
        """Freezes the GRU and predictor to preserve pretrained physics."""
    ...
    def unfreeze_shared_core(self):
        """Unfreezes the GRU and predictor."""
        for param in self.gru.parameters():
